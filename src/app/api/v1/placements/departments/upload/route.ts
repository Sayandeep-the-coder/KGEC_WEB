  ...
  try {
    ...
    if (validRows.length > 0) {
      await db.transaction(async (tx) => {
        ...
        for (const year of affectedYears) {
          ...
          if (deptRecords.length > 0) {
            const totalStudentsPlaced = deptRecords.reduce(
              (acc, curr) => acc + curr.studentsPlaced,
              0
            );

            const maxHighestSalary = deptRecords.reduce(
              (max, curr) => {
                if (
                  curr.highestSalary !== null &&
                  (max === null || curr.highestSalary > max)
                ) {
      ...
      await writeAuditLog({
        ...
        action: "create",
        resource: "placement_departments",
