...
export default function AlumniClient({ initialAlumni }: Props) {
  ...
  const filteredAlumni = initialAlumni.filter((alumnus) => {
    // 1. Category Filter
    if (selectedCat !== "all" && alumnus.category !== selectedCat) {
      return false;
    }
    // 2. Search Filter
    if (searchQuery.trim()) {
      ...
      const matches = 
        ...
        (alumnus.company && alumnus.company.toLowerCase().includes(q)) ||
        (alumnus.highlight && alumnus.highlight.toLowerCase().includes(q));
